...
#args = argparse.Namespace(model='xlmr', layer=8, device='0', dataset='tatoeba', #tatoeba_use_task_order=True,batch_size=16, save_whitened=False, save_cbie=True, dist='cosine', embed_size=768, tgt_language='en')
...
def parse_args():
    ...
    parser.add_argument('-s', '--src_lang', default='hsb',type=str, required=True, help='Source language')
    parser.add_argument('-t', '--trg_lang', default='de',type=str, required=True, help='Target language')

    parser.add_argument('--layer', type=int, default=8, help='Layer to extract embeddings from')
    parser.add_argument('--device', type=str, default='cuda', help='Device to use: "cuda" or "cpu"')
    parser.add_argument('--batch_size', type=int, default=16, help='Batch size for processing')
    parser.add_argument('--embed_size', type=int, default=768, help='Embedding dimension')

    parser.add_argument('--save_whitened', action='store_true', help='Enable whitening transformation')
    parser.add_argument('--save_cbie', action='store_true', help='Enable CBIE transformation')
    parser.add_argument('--dist', type=str, default='cosine', choices=['cosine', 'euclidean'], help='Distance metric')
...
def transform_saved_embeddings(args, dataset_split, device, langs, model, tokenizer):
    ...
    for (src_lang, trg_lang) in langs: # e.g., hsb-de
        ...
        # DATA TO SPECIFY TRAINING ETC
        data = dataset_split # 'test'
        embedding_folder_path = f'./CBIE/{src_lang}-{trg_lang}'
...
def main():
    ...
    model, tokeniser = load_model(args, device=device)
    ...
    for (data, lang), (id_list, tf_embeddings) in embeddings_dict.items():
        ...
        convert_id_embed_to_list(id_list=id_list, embeddings_list=tf_embeddings, 
                output_path=f'CBIE/{src_lang}-{trg_lang}/{output_file_name}', 
                start_i=0)
